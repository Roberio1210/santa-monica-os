...
/** Card da seção "Entradas" da Gestão do Dia — mostra o status ATUAL da ordem (não há histórico de status por dia), qualquer que seja o período filtrado. */
export function EntryCard({ order }: { order: ManagerBoardOrder }) {
  return (
    <Link
    ...
    >
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-2 flex items-center gap-3 text-xs text-foreground-subtle">
          <span>Entrou às {formatClockTime(order.visitCreatedAt)}</span>
