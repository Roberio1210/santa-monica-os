...
import { addDaysIso, comparePeriodValues, previousPeriodOf, saoPauloDateISO, type PeriodComparison, type PeriodRange } from "@/lib/utils/timezone";
...
function dailyBuckets(from: string, to: string): string[] {
 ...
 */
function weeklyBuckets(from: string, to: string): string[] {
  const buckets = new Set<string>();
  let cursor = from;
  while (cursor <= to) {
    buckets.add(weekBucketOf(cursor));
    cursor = addDaysIso(cursor, 7);
  }
  return Array.from(buckets).sort();
}

function weekBucketOf(dateIso: string): string {
...
function monthlyBuckets(monthsBack: number, asOfMonth: string): string[] {
