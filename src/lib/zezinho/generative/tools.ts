    ...
    daily_management_summary: tool({
      ...
      execute: async ({ dia }) => {
        ...
        return {
          ...
          estoque: {
            // Achado real (Missão Z5, chamada real autenticada como admin): o modelo já leu esses
            // 4 grupos corretamente separados, mas mesmo assim apresentou "ok_relevante"/
            // "comprados_recentemente" sob um título de "estoque crítico" na prosa final — os
            // dados estavam certos, só a rotulagem ficou errada. Avisos inline explícitos nos dois
            // grupos saudáveis, mesmo padrão já eficaz usado em aviso_geral/aviso_principais_servicos.
            ...
            aviso_ok_relevante: result.inventoryOkRelevant.length > 0 ? "Estes itens estão SAUDÁVEIS (status ok/sem mínimo definido) — NUNCA os liste como 'estoque crítico', 'ponto de atenção' ou 'precisa repor'. Aparecem aqui só como contexto (usados em serviço vendido hoje ou comprados recentemente)." : null,
            comprados_recentemente: result.recentPurchases.map((p) => ({ produto: p.name, quantidade: p.quantity, unidade: p.unit, data: p.date })),
            aviso_comprados_recentemente: result.recentPurchases.length > 0 ? "Isto é só um registro informativo de compras recentes — NÃO significa que o item está em falta ou precisa de atenção. Só os itens em 'comprar'/'atencao' acima exigem decisão." : null,
          },
          amanha: {
