...
describe("runDiretoria — orquestrador da Reunião de Diretoria (Sprint 5.0, Z1)", () => {
  ...
  it("devolve a Memória Organizacional (Z3B) junto do relatório consolidado", async () => {
    const { organizationalMemory } = await runDiretoria();
    expect(Array.isArray(organizationalMemory.recentLearnings)).toBe(true);
    expect(Array.isArray(organizationalMemory.activeBeliefs)).toBe(true);
    expect(Array.isArray(organizationalMemory.strategicItems)).toBe(true);
