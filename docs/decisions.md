...
| 2026-08-28 | `vitest` nunca usa `DATABASE_URL` (só `TEST_DATABASE_URL`, opcional) — `NODE_ENV === "test"` é checado em `src/db/client.ts`/`src/lib/storage/mode.ts` antes de qualquer módulo dual-mode decidir Postgres vs. memória | Incidente real: rodar a suíte com `.env.local` de produção carregado no processo (ex.: `dotenv-cli -e .env.local`) fez repositórios dual-mode escreverem em produção de verdade, contaminando clientes/veículos/agendamentos/ordens/notificações/memória organizacional; auditado e limpo. Ver `docs/database-and-auth-setup-guide.md` (seção "Como rodar os testes com segurança") |
